...
let config: {
  ...
  labelMappings?: { [key: string]: string }; //map renovate Branch-Context-Name to Gerrit-Label-Name
...
export function initPlatform({
  ...
  password,
  gerritLabelMapping,
}: PlatformParams & AllConfig): Promise<PlatformResult> {
  ...
  config.labelMappings = gerritLabelMapping as { [key: string]: string };
...
function mapBranchStateContextToLabel(context: string | null | undefined): {
  ...
  if (context && config.labelMappings && config.labelMappings[context]) {
    const labelName = config.labelMappings[context];
